...
DEFINE_string(colmap_database_file, "", "Filename of the COLMAP database file (.db).");
...
//    std::cout << *i << ' ';
  ...
  /* All the content from camera_intrinsics_prior */
  /*
  //for (auto i = camera_intrinsics_prior.begin(); i != camera_intrinsics_prior.end(); ++i)
  for( auto i=0; i<camera_intrinsics_prior.size(); ++i)
  {
//    std::cout << camera_intrinsics_prior[i].focal_length << ' ';
    std::cout << "camera_intrinsics_model_type = " << camera_intrinsics_prior.at(i).camera_intrinsics_model_type << ", image size (H*W) = " << camera_intrinsics_prior.at(i).image_height << " * " << camera_intrinsics_prior.at(i).image_width << std::endl;
  }

  theia::CameraIntrinsicsPrior camIntrinsicPrior0 = camera_intrinsics_prior.at(0);
  std::cout << "000 !!! camera_intrinsics_model_type = " << camIntrinsicPrior0.camera_intrinsics_model_type << ", image size (H*W) = " << camIntrinsicPrior0.image_height << " * " << camIntrinsicPrior0.image_width << std::endl;
  std::cout << "0th camera_intrinsics_prior: " << " focal_length = " << camera_intrinsics_prior.at(0).focal_length.value[0] << ", is_set = " << camera_intrinsics_prior.at(0).focal_length.is_set << "; principal_point = (" << camera_intrinsics_prior.at(0).principal_point.value[0] << "," << camera_intrinsics_prior.at(0).principal_point.value[1] << "), is_set = " << camera_intrinsics_prior.at(0).principal_point.is_set << std::endl;


  std::cout << "camera_intrinsics_prior cout done with total camera intrinsics prior number = " << camera_intrinsics_prior.size() << std::endl;
  */
  /* All the content from image_matches */
/*
  //for (auto i = image_matches.begin(); i != image_matches.end(); ++i)
  for( auto i=0; i<image_matches.size(); ++i)
  {
    //std::cout << image_matches[i].image1 << ' ';
    std::cout << image_matches.at(i).image1 << " --- " << image_matches.at(i).image2 << std::endl;
  }
  std::cout << "image_matches cout done! There are totally " << image_matches.size() << " matching image pairs!" << std::endl;
  // print out all contents from image_matches[0]
  std::cout << "The 1st pair: " << image_matches.at(0).image1 << " --- " << image_matches.at(0).image2 << std::endl;

  std::vector<theia::FeatureCorrespondence> correspondences = image_matches.at(0).correspondences;
  for (auto i = image_matches.at(0).correspondences.begin(); i != image_matches.at(0).correspondences.end(); ++i)
  //for (auto i = correspondences.begin(); i != correspondences.end(); ++i)
  {
...
bool isOpenDB = false;
bool ConnectDB(sqlite3 *dbfile, const std::string& DBfilepath)
{
    if ( sqlite3_open(DBfilepath.c_str(), &dbfile) == SQLITE_OK )
    {
        isOpenDB = true;
        return true;
    }
...
void DisonnectDB(sqlite3 *dbfile)
{
    if ( isOpenDB == true )
    {
        sqlite3_close(dbfile);
        std::cout << "opened DB is closed now!" << std::endl;
    }
}
...
static int imagesSQLcallback(void *veryUsed, int argc, char **argv, char **azColName){
  int i;
  std::vector<std::string> *image_files = (std::vector<std::string> *)veryUsed;
  for(i=0; i<argc; i++){
    image_files->at(i) = (argv[1]);
    std::cout<<"argv[1]="<<argv[1]<<std::endl;
  }
  return 0;
}

static int imagenameSQLcallback(void *veryUsed, int argc, char **argv, char **azColName){
  int i;
...
void ImportMatchesFromColmapDB_GenerateTheiaMatchfile(const std::string& FLAGS_colmap_database_file, bool VisContent = false)
{
  ...
  bool DB1connection = false;
  if (FLAGS_colmap_database_file.size() != 0) {
    DB1connection = ConnectDB(dbfile, FLAGS_colmap_database_file);
    if( DB1connection == true )
      {
        std::cout << "Congrats! FLAGS_colmap_database_file: " << FLAGS_colmap_database_file << " is opened successfully!" << std::endl;

        /* Do some SQL operations: data retrieval and store them in Theia objects for later matchfile generation*/
        int rc;
...
//        rc = sqlite3_exec(dbfile, sqlQuery, imagesSQLcallback, &image_files, &zErrMsg);
    ...
    LOG(FATAL) << "Specifiy a colmap_database_file to import the content inside.";
  }

  // Read in match file. (should be commented out when this function is complete)
  theia::ReadMatchesAndGeometry(FLAGS_matches_file,
                                &image_files,
                                &camera_intrinsics_prior,
                                &image_matches);
...
int main(int argc, char *argv[])
{
  ...
  std::cout << "FLAGS_matches_file = " << FLAGS_matches_file << std::endl;
